                    ...
                    <form action="{{ route('product.update', $Edit->id) }}" method="POST">
                        ...
                        <div class="form-row">
                            <div class="col-xl-6 col-sm-6 col-12 form-group">
                                ...
                                <input name="product_name" type="text" value="{{ $Edit->name }}"
                                ...
                                <select name="supplier_name" class="form-control form-control-sm rounded-0">
                                    @foreach ($Supplier as $Suppliers)
                                        <option value="{{ $Suppliers->id }}"
                                            {{ $Suppliers->id == $Edit->supplier->id ? 'selected' : '' }}>
                                ...
                                <select name="category_name" class="form-control form-control-sm rounded-0">
                                    @foreach ($Category as $Categorys)
                                        <option value="{{ $Categorys->id }}"
                                            {{ $Categorys->id == $Edit->category->id ? 'selected' : '' }}>
                                ...
                                <select name="unit_name" class="form-control form-control-sm rounded-0">
                                    @foreach ($Unit as $Units)
                                        <option value="{{ $Units->id }}"
                                            {{ $Units->id == $Edit->unit->id ? 'selected' : '' }}>{{ $Units->name }}
                                        </option>
                                    @endforeach
                                </select>
                                @error('unit_name')
                        ...
                        <div class="form-group form-check">
                            <input type="checkbox" {{ $Edit->status == true ? 'checked' : '' }} class="form-check-input"
