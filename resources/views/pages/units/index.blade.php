    ...
    <div class="card shadow-lg rounded-0">
        <div class="card-header">
            <h3 class="m-0 d-flex justify-content-between">Units List
                <a href="{{ route('units.create') }}" class="btn btn-sm rounded-0 shadow-sm text-light btn-color"><i class="fa fa-plus fa-sm"></i>  Add New</a>
            </h3>
        </div>
        <div class="card-body">
                            ...
                            <td>
                                <div class="dropdown show">
                                    ...
                                    <div class="dropdown-menu dropdown-menu-right" x-placement="top-end" style="position: absolute; will-change: transform; top: 0px; left: 0px; transform: translate3d(-84px, -79px, 0px);">
                                      ...
                                      <a class="dropdown-item" href="{{ route('units.edit', $Unit->id) }}"> Edit </a>

                                        <button class="dropdown-item" style="cursor: pointer;" type="button" onclick="delete_units({{ $Unit->id }})">Delete
                                        </button>

                                        <form id="delete-units-{{ $Unit->id }}" action="{{ route('units.destroy',$Unit->id) }}" method="POST">
                                            @csrf
                                            @method('DELETE')
                                        </form>
